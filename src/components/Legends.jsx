...
const Legends = ({ user }) => {
  ...
  useEffect(() => {
    ...
    const unsubscribe = onSnapshot(q, (snapshot) => {
      ...
      console.error("Legends listener error:", error);
      setLoading(false);
    });
    return () => unsubscribe();
  }, [user.uid]);
  ...
  return (
    <div className="flex flex-col gap-6 animate-in slide-in-from-right duration-500 p-4">
      <div>
        ...
        <p className="text-body-md text-on-surface-variant">Suas figurinhas mais prestigiadas.</p>
      </div>
      ...
      {stickers.length === 0 && (
        <div className="text-center py-20 bg-surface-container/30 rounded-2xl border-2 border-dashed border-outline-variant">
          ...
          <p className="text-body-lg text-on-surface-variant italic">Nenhuma Legend ainda.</p>
        </div>
      )}
    </div>
  );
